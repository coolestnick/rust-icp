...
#[update]
fn add_message(message: MessagePayload) -> Result<Message, Error> {
    let id = ID_COUNTER
        .with(|counter| {
...
fn get_all_messages() -> Vec<Message> {
    STORAGE.with(|service| {
        service.borrow().iter().map(|(_, message)| message.clone()).collect()
    })
}

#[query]
fn search_messages_by_title(query: String) -> Vec<Message> {
    STORAGE.with(|service| {
        service
            .borrow()
            .iter()
            .filter(|(_, message)| message.title.contains(&query))
...
#[query]
fn get_messages_paginated(page: u64, page_size: u64) -> Vec<Message> {
    STORAGE.with(|service| {
        service
            .borrow()
            .iter()
